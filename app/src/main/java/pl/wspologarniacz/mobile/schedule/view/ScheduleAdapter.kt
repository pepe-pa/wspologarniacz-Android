...
class ScheduleAdapter : SimpleListAdapter<Schedule>(ScheduleDiffCallback) {
    ...
    inner class ScheduleViewHolder(view: View) : BindViewHolder<Schedule>(view) {
        ...
        private val membersSize =
                containerView.context.resources.getDimensionPixelSize(R.dimen.schedule_users_height)
        ...
        override fun bind(item: Schedule) {
            ...
            users.addChildren(item.members)
            modified.text = item.modified
        }


        private fun LinearLayout.addChildren(members: List<Member>) {


            removeAllViews()
            members.forEach {
                val avatarImageView = LayoutInflater.from(containerView.context).inflate(R.layout.small_avatar_layout, this, false) as ImageView
